...
if [ -z ${port} ] || [ -z ${user_name} ] || [ -z ${user_pass} ]; then
        echo "Usage: $0 port user_name user_pass [noaptupdate]"
        exit 1
fi
...
#ch###################################################################################################
# теперь пользователям списка auth_users разрешен доступ к интернету
...
htpasswd -b -c /etc/squid/squidusers ${user_name} ${user_pass}
